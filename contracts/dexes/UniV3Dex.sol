...
contract UniV3Dex is ILiquidityDex, Ownable {
  ...
  function doSwap(
    ...
    IERC20(currentSellToken).safeTransferFrom(spender, address(this), amountIn);
    IERC20(currentSellToken).safeIncreaseAllowance(uniswapV3Router, amountIn);
    ...
    uint256 actualAmountOut = ISwapRouter(uniswapV3Router).exactInput(param);
    return actualAmountOut;
  }

  function pairFee(address sellToken, address buyToken) public view returns(uint24 fee) {
  ...
  function setFee(address token0, address token1, uint24 fee) public onlyOwner {
